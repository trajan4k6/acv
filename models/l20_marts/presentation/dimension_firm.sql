...
UNION
SELECT F.DIMENSION_FIRM_KEY,TO_CHAR(F.LEGACY_FIRM_ID), F.FIRM_NAME,  F.ACCOUNT_ID, AC.ACCOUNT_CLASSIFICATION, A.CRM_FIRM_TYPE, NULL AS FIRM_CATEGORY, AR.REGION_NAME, ART.REGION_TEAM_NAME, NULL, NULL, CASE A.Account_Status WHEN 'Active' THEN true WHEN 'Inactive' THEN false END Is_Active, COALESCE(A.DIMENSION_ACCOUNT_CLASSIFICATION_KEY, '-1') DIMENSION_ACCOUNT_CLASSIFICATION_KEY, COALESCE(A.DIMENSION_REGION_KEY, '-1') DIMENSION_REGION_KEY, COALESCE(A.DIMENSION_REGION_TEAM_KEY, '-1') DIMENSION_REGION_TEAM_KEY, F.DATASOURCE_ID
...
fd.parent_firm_name,
fd.Is_Active,
case when cpf.dimension_firm_key is not null then true else false end is_customer,
case when is_customer = true or cpsf.dimension_firm_key is not null then true else false end is_parent_logo,
