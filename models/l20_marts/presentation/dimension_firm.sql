...
SELECT '-1' AS DIMENSION_FIRM_KEY, NULL AS CRM_FIRM_ID, NULL FIRM_NAME, NULL SALESFORCE_ACCOUNT_ID, NULL AS ACCOUNT_CLASSIFICATION, NULL AS REGION_NAME, NULL AS REGION_TEAM_NAME, '-1' AS DIMENSION_ACCOUNT_CLASSIFICATION_KEY, '-1' AS DIMENSION_REGION_KEY, '-1' AS DIMENSION_REGION_TEAM_KEY, NULL AS DATASOURCE_ID
UNION
--1.Primary Firm list from Core
SELECT F.DIMENSION_FIRM_KEY, TO_CHAR(F.FIRM_ID) AS CRM_FIRM_ID, F.FIRM_NAME, A.ACCOUNT_ID AS SALESFORCE_ACCOUNT_ID, AC.ACCOUNT_CLASSIFICATION, AR.REGION_NAME, ART.REGION_TEAM_NAME, COALESCE(A.DIMENSION_ACCOUNT_CLASSIFICATION_KEY, '-1') DIMENSION_ACCOUNT_CLASSIFICATION_KEY, COALESCE(A.DIMENSION_REGION_KEY, '-1') DIMENSION_REGION_KEY, COALESCE(A.DIMENSION_REGION_TEAM_KEY, '-1') DIMENSION_REGION_TEAM_KEY, F.DATASOURCE_ID
...
UNION 
SELECT A.DIMENSION_ACCOUNT_KEY, TO_CHAR(A.CRM_FIRM_ID), A.ACCOUNT_NAME, A.ACCOUNT_ID, AC.ACCOUNT_CLASSIFICATION, AR.REGION_NAME, ART.REGION_TEAM_NAME, A.DIMENSION_ACCOUNT_CLASSIFICATION_KEY, A.DIMENSION_REGION_KEY, A.DIMENSION_REGION_KEY, A.DATASOURCE_ID
