...
(SELECT
 ...
 , LAG(ACV_GBP) OVER (PARTITION BY FIRM_ID,LOOKUP_ID,FIRM_TIER,PARENT_FIRM_TYPE,FIRM_CATEGORY,INVENTORY_DESCRIPTION,ASSET_CLASS,PRODUCT_TYPE,REGION,COUNTRY,STATE,CURRENCY  ORDER BY AS_AT_DATE) PRE_PRICE_ACV --PRICE_INC_DEC_CALC
 ...
 , LAG(START_DATE) OVER (PARTITION BY FIRM_ID,LOOKUP_ID,FIRM_TIER,PARENT_FIRM_TYPE,FIRM_CATEGORY,INVENTORY_DESCRIPTION,ASSET_CLASS,PRODUCT_TYPE,REGION,COUNTRY,STATE,CURRENCY,START_DATE,END_DATE  ORDER BY AS_AT_DATE) PRE_NO_CHANGE_START_DATE --NO_CHANGE_CALC
 ...
 , LAG(LOGO.LOGO_START_DATE) OVER (PARTITION BY FIRM_ID,LOOKUP_ID,FIRM_TIER,PARENT_FIRM_TYPE,FIRM_CATEGORY,INVENTORY_DESCRIPTION,ASSET_CLASS,PRODUCT_TYPE,REGION,COUNTRY,STATE,CURRENCY  ORDER BY AS_AT_DATE) PRE_START_DATE --FLAT_RENEWAL_CALC
 , LAG(LOGO.LOGO_START_DATE) OVER (PARTITION BY FIRM_ID,LOOKUP_ID,FIRM_TIER,PARENT_FIRM_TYPE,FIRM_CATEGORY,INVENTORY_DESCRIPTION,ASSET_CLASS,PRODUCT_TYPE,REGION,COUNTRY,STATE,CURRENCY  ORDER BY AS_AT_DATE) PRE_END_DATE
 , LOGO.COUNT_START_DATE COUNT_START_DATE --FLAT_RENEWAL_CALC
 ...
 , REGION,COUNTRY,STATE,SALES_REGION
 , CURRENCY,ACV_GBP
 , START_DATE,END_DATE
 FROM
(SELECT FIRM_ID,LOOKUP_ID,FIRM_NAME,FIRM_TIER,PARENT_FIRM_TYPE,FIRM_CATEGORY,INVENTORY_DESCRIPTION,ASSET_CLASS,PRODUCT_TYPE,REGION,SALES_REGION,COUNTRY,STATE,'GBP' CURRENCY,MAX(START_DATE) START_DATE,MAX(END_DATE) END_DATE,
		AS_AT_DATE,SUM(ACV_GBP) ACV_GBP
 FROM   {{ref('stg_acv_values')}}  
 WHERE PRODUCT_TYPE ='Subscription'
GROUP BY FIRM_ID,LOOKUP_ID,FIRM_NAME,FIRM_TIER,PARENT_FIRM_TYPE,FIRM_CATEGORY,INVENTORY_DESCRIPTION,ASSET_CLASS,PRODUCT_TYPE,REGION,SALES_REGION,COUNTRY,STATE,AS_AT_DATE--,CURRENCY,START_DATE,END_DATE
)
 ...
 (SELECT LOOKUP_ID LOGO_LOOKUP_ID,
         ...
         NVL(LAG(START_DATE) OVER (PARTITION BY LOOKUP_ID ORDER BY AS_AT_DATE),START_DATE) LOGO_PRE_START_DATE,
         NVL(LEAD(START_DATE) OVER (PARTITION BY LOOKUP_ID ORDER BY AS_AT_DATE),START_DATE) LOGO_POST_START_DATE,
         ...
         START_DATE LOGO_START_DATE,
         ACV LOGO_CUR_ACV,
         COUNT_START_DATE
  FROM 
        (SELECT LOOKUP_ID,AS_AT_DATE,MAX(START_DATE) START_DATE,SUM(ACV_GBP) ACV,COUNT(DISTINCT START_DATE) COUNT_START_DATE FROM   {{ref('stg_acv_values')}}   WHERE PRODUCT_TYPE ='Subscription' GROUP BY LOOKUP_ID,AS_AT_DATE)
 --WHERE LOOKUP_ID='650'
...
SELECT  --'1' FLAG,
    ...
    LOGO_MAX_AS_AT_DATE,FIRM_MAX_AS_AT_DATE,FIRM_POST_AS_AT_DATE,
       --LOGO_PRE_AS_AT_DATE,LOGO_PRE_ACV,LOGO_CUR_ACV , PRE_ACV, PRE_START_DATE,
        FIRM_ID,
        LOOKUP_ID,
        FIRM_NAME,
        FIRM_TIER,
        ...
        0 CHURN,
        CASE WHEN MONTHS_BETWEEN(AS_AT_DATE,LOGO_PRE_AS_AT_DATE)=1 AND LOGO_PRE_ACV=LOGO_CUR_ACV AND PRE_PRICE_ACV=ACV_GBP AND START_DATE<>PRE_START_DATE AND COUNT_START_DATE =1 THEN 1 ELSE 0 END FLAT_RENEWAL,
        CASE WHEN MONTHS_BETWEEN(AS_AT_DATE,PRE_AS_AT_DATE)=1  AND PRE_PRICE_ACV=ACV_GBP  AND START_DATE=PRE_NO_CHANGE_START_DATE  THEN 1 ELSE 0 END FOLLOW,
        ...
        0 LOCATION_DROP,
        CASE WHEN MONTHS_BETWEEN(AS_AT_DATE,PRE_AS_AT_DATE)=1 AND PRE_PRICE_ACV<ACV_GBP THEN 1 ELSE 0 END PRICE_INCREASE,
        CASE WHEN MONTHS_BETWEEN(AS_AT_DATE,PRE_AS_AT_DATE)=1 AND PRE_PRICE_ACV>ACV_GBP THEN 1 ELSE 0 END PRICE_DECREASE,
